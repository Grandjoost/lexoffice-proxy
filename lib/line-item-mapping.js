...
export function mapLineItemToABPosition(lineItem, taxRatePercentage) {
  ...
  if (isRecurring) {
    ...
    // quantity = termMonate × menge (HubSpot-Logik: N Pakete pro Monat über M Monate = N×M Stück)
    const position = {
      type: 'custom',
      name: lineItem.name + ' (' + formatMMYYYY(start) + ' – ' + formatMMYYYY(end) + ')',
      quantity: termMonths * quantity,
      unitName: 'Stück',
      unitPrice: {
        currency: 'EUR',
        netAmount: price,
...
export function mapLineItemToInvoicePosition(lineItem, taxRatePercentage, monthStart) {
  ...
  if (isRecurring) {
    // quantity = menge (Pakete pro Monat). Summiert über alle Monatsrechnungen = AB-Quantity.
    const position = {
      type: 'custom',
